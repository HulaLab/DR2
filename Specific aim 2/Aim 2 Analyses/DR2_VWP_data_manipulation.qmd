...
df3 %>% 
  na.omit() %>%
  ggplot(aes(x = BIN_INDEX, y = count)) +
...
Mirman et al code the counts as TRUE OR FALSE for looks at a quadrant. So what i did here was change all 0s, meaning no looks to that quadrant at a given time bin to FALSE and all numbers \> 0 as TRUE\>

```{r}

df3 = df3 %>%
  mutate(count = ifelse(count != 0, TRUE, FALSE))
```
...
df4 = df3 %>%
  ...
  summarize(sumFix = sum(count), nTrials = unique(nTrials),
            meanFix = sum(count)/unique(nTrials)) %>%
  na.omit(meanFix)
```
