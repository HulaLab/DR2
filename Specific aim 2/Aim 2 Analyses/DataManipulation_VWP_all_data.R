...
file_list = list.files(path = here("all_data/updated_xls_format"), pattern = "^3086", full.names = TRUE)

library(purrr)
library(readxl)
library(dplyr)

file_list = list.files(path = here("all_data/updated_xls_format"),  full.names = TRUE)

df <- file_list %>%
  map_df(function(file) {
    data <- read_excel(file, col_types = c(RECORDING_SESSION_LABEL = "text"))
    return(data)
  })
...
df = df %>%
  ...
  select(participant, session, BIN_INDEX, 
         fixation_location, count, quadrant_stimuli_name, quadrant_number)
  

#remove practice and create a general area of interst variable
#---DONE
#aoi1-4 combines the left and right eye sample count data
#---done

df2 = df %>%
  filter(condition != 'practice') %>%
  mutate(aoi1 = ifelse(RIGHT_IA_1_SAMPLE_COUNT == '.', 
                       LEFT_IA_1_SAMPLE_COUNT,
                       RIGHT_IA_1_SAMPLE_COUNT)) %>%
  mutate(aoi2 = ifelse(RIGHT_IA_2_SAMPLE_COUNT == '.', 
                       LEFT_IA_2_SAMPLE_COUNT, 
                       RIGHT_IA_2_SAMPLE_COUNT)) %>%
  mutate(aoi3 = ifelse(RIGHT_IA_3_SAMPLE_COUNT == '.', 
                       LEFT_IA_3_SAMPLE_COUNT,
                       RIGHT_IA_3_SAMPLE_COUNT)) %>%
  mutate(aoi4 = ifelse(RIGHT_IA_4_SAMPLE_COUNT == '.', 
...
df2.02 = df2.01 %>%
  ...
  mutate(max_bin = max(as.numeric(BIN_INDEX))*25)

test = test %>%
  mutate(check_na = is.na(count_by_bin_by_quadrant_content)) 

test = df %>%
  mutate(check_nan = is.na(unrelated_location))
